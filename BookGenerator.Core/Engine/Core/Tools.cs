...
using BookGenerator.Core.Engine.Core.Functions;
using BookGenerator.Core.Engine.Core.Interop;
using BookGenerator.Core.Engine.Extensions;
using JSBool = BookGenerator.Core.Engine.BaseLibrary.Boolean;
    ...
    public static class Tools
    {
                ...
                && typeof(IEnumerable).IsAssignableFrom(targetType))
            {
                     ...
                     && targetType.IsAssignableFrom((elementType = @interface.GetGenericArguments()[0]).MakeArrayType())))
                {
                    if (elementType.GetTypeInfo().IsPrimitive)
                    {
                        ...
                        if (value is TypedArray ta && ta.ElementType == elementType)
        ...
        public static string DoubleToString(double d)
        {
            ...
            lock (cachedDoubleString)
            {
                ...
                {
                    var neg = (d < 0.0 || (d == -0.0 && Tools.IsNegativeZero(d))) ? 1 : 0;
        ...
        internal static BaseLibrary.Array arraylikeToArray(JSValue src, bool evalProps, bool clone, bool reassignLen, long _length)
        {
            ...
            {
                goDeep = false;
                if (src is BaseLibrary.Array srca)
                {
                    if (_length == -1)
        ...
        internal static JSValue InvokeGetter(JSValue property, JSValue target)
        {
            ...
            if (!(property._oValue is PropertyPair getter) || getter.getter == null)
        ...
        public static string GetTypeName(JSValue v)
        {
            ...
            switch (v._valueType)
            {
                ...
                case JSValueType.Property:
                    {
                        if (v._oValue is PropertyPair prop)
                        {
                            var tempStr = "";
                            if (prop.getter != null)
                                tempStr += "Getter";
        ...
        internal static string JSValueToObjectString(JSValue v, int maxRecursionDepth, int recursionDepth = 0)
        {
            ...
            switch (v.ValueType)
            {
                ...
                case JSValueType.Function:
                    {
                        if (!(v.Value is BaseLibrary.Function f))
                            return v.ToString();
                ...
                case JSValueType.Object:
                    {
                            ...
                            || v == Context.CurrentGlobalContext.GetPrototype(typeof(BaseLibrary.Array)))
                        {
                            ...
                            if (!(v.Value is BaseLibrary.Array a)) // v == Array.prototype
                            {
                        ...
                        {
                            var typeName = Tools.GetTypeName(v);
                            if (recursionDepth >= maxRecursionDepth)
                                return typeName;
                ...
                while (pos < f.Length && hasNext)
                {
                    ...
                    switch (c)
                    {
                        ...
                        case 'd':
                            ...
                            var dstr = Tools.DoubleToString(System.Math.Abs(d));
                            if (d < 0)
                                s.Append('-');
                            if (dstr.Length < para)
