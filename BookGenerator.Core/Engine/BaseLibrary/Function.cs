...
namespace BookGenerator.Core.Engine.BaseLibrary
{
    ...
    public partial class Function : JSObject, ICallable
    {
        ...
        public Function(Arguments args)
        {
            ...
            for (var i = 0; i < len; i++)
                argn += args[i] + (i + 1 < len ? "," : "");
            var code = "function (" + argn + "){" + Environment.NewLine + (len == -1 ? "undefined" : args[len]) + Environment.NewLine + "}";
        ...
        internal void initParameters(Arguments args, bool storeVariablesIntoContext, Context internalContext)
        {
            ...
            var min = System.Math.Min(args._iValue, _functionDefinition.parameters.Length - (_functionDefinition._functionInfo.ContainsRestParameters ? 1 : 0));
