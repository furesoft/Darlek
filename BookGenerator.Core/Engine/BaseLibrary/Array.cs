	...
	public sealed class Array : JSObject, IIterable
	{
		...
		public static JSValue shift(JSValue self)
		{
			...
			else
			{
				...
				var _length = (long)(uint)Tools.JSObjectToDouble(lenObj);
		...
		private static JSValue spliceImpl(JSValue self, Arguments args, bool needResult)
		{
			...
			if (self is Array selfa)
			{
				...
				var pos0 = (long)System.Math.Min(Tools.JSObjectToDouble(args[0]), _length); // double потому, что нужно "с заполнением", а не "с переполнением"
				long pos1 = 0;
				if (args.Length > 1)
				{
			...
			{
				var _length = Tools.getLengthOfArraylike(self, false);
