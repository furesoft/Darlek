...
namespace BookGenerator.Core.Engine.Expressions
{
    ...
    public sealed class ExpressionTree : Expression
    {
        ...
        private static Expression deicstra(ExpressionTree statement)
        {
            ...
            if (!(statement._right is ExpressionTree cur))
                return statement;

            var stats = new Stack<Expression>();
        ...
        private static Expression parseOperand(ParseInfo state, ref int i, bool forNew, bool forForLoop)
        {
            ...
            if (operand == null)
            {
                ...
                {
                    var value = state.Code.Substring(start, i - start);
                    if ((value[0] == '\'') || (value[0] == '"'))
                    {
                    ...
                    else
                    {
                        ...
                        {
                            operand = new Constant(b ? BaseLibrary.Boolean.True : BaseLibrary.Boolean.False) { Position = start, Length = i - start };
                        }
                        else
                        {
                            var n = 0;
