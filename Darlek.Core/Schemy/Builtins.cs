...
public class Builtins
{
    public static IDictionary<Symbol, object> CreateBuiltins(Interpreter interpreter)
    {
        ...
        builtins[Symbol.FromString(">")] = NativeProcedure.Create<double, double, bool>((x, y) => x > y, ">");
        builtins[Symbol.FromString(">=")] = NativeProcedure.Create<double, double, bool>((x, y) => x >= y, ">=");
        builtins[Symbol.FromString("eq?")] = NativeProcedure.Create<object, object, bool>((x, y) => ReferenceEquals(x, y), "eq?");
        ...
        builtins[Symbol.CONS] = NativeProcedure.Create<object, List<object>, List<object>>((x, ys) => (new[] { x }).Concat(ys).ToList(), "cons");
        builtins[Symbol.FromString("not")] = NativeProcedure.Create<bool, bool>(x => !x, "not");
        builtins[Symbol.APPEND] = NativeProcedure.Create<List<object>, List<object>, List<object>>((l1, l2) => l1.Concat(l2).ToList(), "append");
        builtins[Symbol.FromString("null")] = NativeProcedure.Create(() => (object)null, "null");
        builtins[Symbol.FromString("nil")] = NativeProcedure.Create(() => (object)null, "nil");
        builtins[Symbol.FromString("null?")] = NativeProcedure.Create<object, bool>(x => x is List<object> && ((List<object>)x).Count == 0, "null?");
        builtins[Symbol.FromString("nil?")] = NativeProcedure.Create<object, bool>(x => x is List<object> && ((List<object>)x).Count == 0, "nil?");
