...
public class Interpreter
{
    ...
    public static object Read(InPort port)
    {
        ...
        readAhead = token => {
            ...
            else if (token is string)
            {
                ...
                else if (Symbol.QuotesMap.TryGetValue(tokenStr, out var quote))
                {
        ...
        expand = (x, topLevel) => {
            ...
            else if (Symbol.DEFINE.Equals(xs[0]) || Symbol.DEFINE_MACRO.Equals(xs[0]))
            {
                ...
                if (v is List<object>) // defining function: ([define|define-macro] (f arg ...) body)
                {
                    ...
                    return expand(new List<object> { def, f, (new object[] { Symbol.LAMBDA, @params }).Concat(body).ToList() }, false);
                }
            ...
            else if (Symbol.LAMBDA.Equals(xs[0]))
            {
                ...
                {
                    // (lambda (...) expr+
                    body = (new[] { Symbol.BEGIN }).Concat(xs.Skip(2)).ToList();
                }
    ...
    public static object EvaluateExpression(object expr, Environment env)
    {
        while (true)
        {
            ...
            else
            {
                ...
                {
                    for (var i = 1; i < exprList.Count - 1 /* don't eval last expr yet */; i++)
                    {
    ...
    public void AddMacro(string name, Procedure proc)
    {
        macroTable.Add(Symbol.FromString(name), proc);
    }
    ...
    {
        using (var reader = new StreamReader(new MemoryStream(Resources.init)))
        {
            yield return reader;
        }
