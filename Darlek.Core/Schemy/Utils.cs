...
public static class Utils
{
    ...
    public static void CheckSyntax(object expr, bool success, string msg = null)
    {
        ...
        if (!success)
        {
            throw new SyntaxError(string.Format("{0}: {1}", msg, PrintExpr(expr)));
        }
    }
    ...
    public static T ConvertType<T>(object val)
    {
        ...
        {
            throw new InvalidOperationException(string.Format("Cannot convert {0} to type {1}", PrintExpr(val), typeof(T).Name));
        }
    }
